...
(function($){
	$.fn.fileStyle = function(options, refreshOptions) {
		...
		base.initFileStyle = function(element){
			...
			element.parent().append('<div class="file-overlay"><img src="/images/file.jpg" alt="Choose File" /><div></div><br style="clear:both" /></div><br style="clear:both" />');
		...
		/*
		 * Commented out until it works properly.
		 */
...
//			element.parent().find('.select-overlay').html('<span class="select-title">'+element.find('option:selected').html()+'</span>').append('<div><ul></ul></div>');
//			if ($.browser.msie && $.browser.version.substr(0,1)<7) {
//				element.hide();
//			}
//			element.children('option').each(function(){
//				element.parent().children('.select-overlay').children('div').children('ul').append('<li>' + $(this).html() + '</li>');
//			})
		};
		...
		base.wrapElement = function(){
			base.each(function(){
				if (this.tagName == 'SELECT') {
					base.initSelectStyle($(this));
				}
				else {
					switch($(this).attr('type')) {
						case 'radio':
							base.initRadioStyle($(this));
							break;
						case 'checkbox':
							base.initCheckboxStyle($(this));
							break;
						case 'text':
							break;
						case 'textarea':
							break;
						case 'select-one':
							// No longer works as of jQuery 1.6.2
							base.initSelectStyle($(this));
							break;
						case 'file':
							base.initFileStyle($(this));
							break;
						default:
							break;
					}
		...
		base.initButtons = function(){
			$('.select-overlay').toggle(function(){
				...
				$('.select-overlay li').click(function(){
					...
					$(this).parent().parent().parent().parent().find('select').children('option').removeAttr('selected');
